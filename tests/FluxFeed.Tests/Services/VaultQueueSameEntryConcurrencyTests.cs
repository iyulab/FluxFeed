...
public class VaultQueueSameEntryConcurrencyTests : IDisposable
{
    ...
    public async Task DequeueAsync_DoesNotHandOutASecondJobForAnEntryAlreadyInFlight()
    {
        ...
        await queue.EnqueueMemorizeAsync(hash, path);
        await queue.EnqueueRefreshAsync(hash, path);
        ...
        var afterCompletion = await queue.DequeueAsync();
    ...
    {
        var queue = CreateService();
        await queue.EnqueueMemorizeAsync("hash-a", Path.Combine(_testDir, "a.md"));
        await queue.EnqueueMemorizeAsync("hash-b", Path.Combine(_testDir, "b.md"));
    ...
    public async Task EnqueueAsync_CoalescesASecondQueuedJobOfTheSameTypeForTheSameEntry()
    {
        ...
        var first = await queue.EnqueueMemorizeAsync(hash, path);
        var second = await queue.EnqueueMemorizeAsync(hash, path);
        ...
        var queued = await queue.GetJobsAsync(VaultJobStatus.Queued);
    ...
    public async Task EnqueueAsync_RaisesThePriorityOfTheJobItCoalescesInto()
    {
        ...
        await queue.EnqueueMemorizeAsync(hash, path, VaultJobPriority.Low);
        var escalated = await queue.EnqueueMemorizeAsync(hash, path, VaultJobPriority.High);
    ...
    public async Task EnqueueAsync_DoesNotCoalesceIntoAJobThatIsAlreadyProcessing()
    {
        ...
        await queue.EnqueueMemorizeAsync(hash, path);
        var inFlight = await queue.DequeueAsync();
        inFlight.Should().NotBeNull();

        var followUp = await queue.EnqueueMemorizeAsync(hash, path);
